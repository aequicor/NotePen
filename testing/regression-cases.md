...
| ID | Sev | Tier | Type | Scenario (RU) | Expected | Source | Coverage | Status |
|---|---|---|---|---|---|---|---|---|
| RC-PEXP-001 | 🟠 | 1 | unit | PageExtent.expandedLeft/Right расширяют соответствующую границу на fraction от текущей ширины; компаундирование корректно | expandedRight(1/3) → right=4/3; два раза → right=16/9; invariants held; top/bottom unchanged. | release/1.0.17 | drawing/api/src/commonTest/kotlin/ru/kyamshanov/notepen/annotation/domain/model/PageExtentExpandTest.kt | PEND |
| RC-PEXP-002 | 🟠 | 1 | unit | PdfDrawingState клампирует X к extent.left/right, Y не клампирует; expand меняет extent и бампит historyVersion | startDrawing/addPoint с x=1.5 сохраняет x=1f; у ≠ 1.2 остаётся 1.2; expandRight меняет right и bumps historyVersion. | release/1.0.17 | drawing/api/src/commonTest/kotlin/ru/kyamshanov/notepen/drawing/api/PdfDrawingStateClampTest.kt | PEND |
| RC-PEXP-003 | 🟠 | 1 | unit | MultiPageDrawingController клампирует ny к 0/1 на крайних страницах; handBoundary инжектирует ny>1 (cap-hiding); isInsidePdfPage учитывает extent | onDown выше page 0 → ny≥0; onMove ниже last page → ny≤1; переход page 0→1 → финальная ny старой страницы >1; isInsidePdfPage false за default extent, true в расширенном. | release/1.0.17 | drawing/impl/src/commonTest/kotlin/ru/kyamshanov/notepen/MultiPageDrawingControllerClampTest.kt | PEND |
| RC-PEXP-004 | 🟡 | 2 | visual | Кнопки "+" расположены полностью за границей страницы; при расширении видна рамка-прямоугольник по границе extent; без расширения рамки нет | Buttons outside page: left button right-edge ≤ page left-edge; right button left-edge ≥ page right-edge. After right-expand: thin grey rectangular border at extent bounds, expanded margin has same paper background; buttons shifted and still outside. Default extent → no border. | release/1.0.17 | → AV-PEXP-01 | PEND |
| RC-PEXP-005 | 🟠 | 2 | visual | В режиме разворота (SPREAD) кнопка "+" показывается только на внешней стороне каждой страницы пары | Левая страница пары → только LEFT "+" кнопка; правая → только RIGHT; нечётная «висячая» — LEFT. | release/1.0.17 | → AV-PEXP-01 | PEND |
| RC-PEXP-006 | 🟡 | 2 | visual | В режиме разворота рамка-extent отображается только для расширенной страницы; у соседней без расширения рамки нет | Если только правая страница расширена — рамка только вокруг правой; левая без рамки. | release/1.0.17 | → AV-PEXP-01 | PEND |
| RC-PEXP-007 | 🟠 | 2 | visual | Рамка-extent не отрывается от страницы при оверскролле | При drag за границу контента (оверскролл) рамка двигается вместе со страницей, зазора между ними нет. | release/1.0.17 | → AV-PEXP-01 | PEND |
| RC-PEXP-008 | 🟠 | 1 | unit | SPREAD: переключение в двух-страничный режим не скрывает увеличенный холст (panForPageTop якорит внешнюю границу) | panForPageTop в SPREAD: левое расширение — левая граница на x=0 вьюпорта; правое расширение — правая граница на x=viewportWidth; без расширений — стандартное центрирование по rowWidth. | release/1.0.17 | rendering/impl/src/commonTest/kotlin/ru/kyamshanov/notepen/pdfviewer/PdfPagesLayoutTest.kt | PASS |
| RC-PEXP-009 | 🟠 | 1 | unit | SPREAD: расширение правого края ЛЕВОЙ страницы пары центрирует полный span (leftExt.right > 1) | panForPageTop SPREAD с leftExt.right=1.3: pan.x = (vp - (1.3*base + GUTTER + base)*zoom) / 2; не падает в else-ветку с неучтённым расширением. | release/1.0.17 | rendering/impl/src/commonTest/kotlin/ru/kyamshanov/notepen/pdfviewer/PdfPagesLayoutTest.kt | PASS |
| RC-PEXP-010 | 🔴 | 2 | crash | EditorPanel: derivedStateOf «активная страница» не падает с ArrayIndexOutOfBoundsException, когда layout временно пуст (pdfHeightsPx.size == 0), а pages ≠ empty | Открыть документ → перезагрузить layout (смена режима или пересчёт страниц): crash отсутствует, активная страница = 0 (fallback). Fix: итерация по lo.pdfHeightsPx.indices вместо pages.indices. | release/1.0.17 | needed | PASS |
| RC-PEXP-011 | 🟠 | 1 | unit | SPREAD: правая страница пары начинается за правым краем слота левой (с учётом extent-расширения); перекрытие отсутствует | build() SPREAD с leftExt.right=4/3: pageLeftsPx[1] = (4/3)*base + GUTTER; pageLeftsPx[1] ≥ leftExt.right*base (нет перекрытия). При стандартном extent (right=1) — результат идентичен прежнему (base + GUTTER). | release/1.0.17 | rendering/impl/src/commonTest/kotlin/ru/kyamshanov/notepen/pdfviewer/PdfPagesLayoutTest.kt | PASS |
...
| RC-REFLOW-013 | 🟠 | 1 | unit | Тогл переноса слов: paragraphStyle/footnoteStyle используют LineBreak.Paragraph при hyphenation=true (Strategy.Simple не активирует Hyphens.Auto) | hyphenation=true → hyphens=Hyphens.Auto, lineBreak=LineBreak.Paragraph; hyphenation=false → hyphens=Hyphens.None, lineBreak=LineBreak.Unspecified (для обоих стилей). | pending | reflow/impl/src/commonTest/kotlin/ru/kyamshanov/notepen/reflow/ui/HyphenationStyleTest.kt | PEND |
| RC-REFLOW-014 | 🟠 | 1 | unit | Алгоритм Кнута-Лиэнга: разбор паттернов (буквы+веса), нечётный вес→перенос, чётный→нет, исключения перекрывают паттерны, leftMin/rightMin отсекают края | parsePattern(«а1б»)→(«аб»,[0,1,0]); на синтетических паттернах разрывы только при нечётном весе и не у краёв. | pending | reflow/impl/src/commonTest/kotlin/ru/kyamshanov/notepen/reflow/ui/LiangHyphenatorTest.kt | PEND |
| RC-REFLOW-017 | 🟠 | 1 | unit | Реальные паттерны hyph-ru (Лиэнг, из ресурсов): корректные слогоразбиения, латиница не трогается | мо-ло-ко, ком-пью-тер; отменяет/хитрость/проблема без прежних кластерных ошибок; Observable без переносов; дом/стол — без переносов. | pending | reflow/impl/src/jvmTest/kotlin/ru/kyamshanov/notepen/reflow/ui/RuHyphenationTest.kt | PASS |
| RC-REFLOW-015 | 🔴 | 1 | unit | Карта мягких переносов plain↔hyph взаимно обратима (стык PLAIN-контракта и HYPH-раскладки TextLayoutResult) | hyphToPlain(plainToHyph(p))==p для всех p; вставка U+00AD на позициях; hyphenated без U+00AD == plain. | pending | reflow/impl/src/commonTest/kotlin/ru/kyamshanov/notepen/reflow/ui/ReaderHyphenationTest.kt | PEND |
| RC-REFLOW-016 | 🔴 | 1 | integration | ~~Десктоп (Skiko): на строке, разорванной по мягкому переносу, дорисованный дефис ВИДЕН (движок сам его не рисует)~~ RETIRED: дорисовка поверх раскладки удалена — дефис теперь НАСТОЯЩИЙ глиф через промоушен; преемник RC-REFLOW-019 | — | pending | ~~DesktopHyphenRenderTest.kt удалён~~ | RETIRED |
| RC-REFLOW-018 | 🟠 | 1 | unit | Промоушен дефисов (fixed-point): promote слота на конце строки, blacklist невместившегося дефиса (2-цикл терминируется), demote/re-promote при каскадном сдвиге, кап → demote-only добивание | Финал детерминирован; последний вызов оракула — для финального варианта; blacklist строго сокращает кандидатов; видимый дефис вне конца строки невозможен. | pending | reflow/impl/src/commonTest/kotlin/ru/kyamshanov/notepen/reflow/ui/ReaderHyphenPromotionTest.kt | PASS |
| RC-REFLOW-019 | 🔴 | 1 | integration | Десктоп (Skiko): слогоразорванная строка оканчивается НАСТОЯЩИМ дефисом, который рисует сам движок; при JUSTIFY дефис заподлицо с краем колонки; правее края колонки чернил нет; голых U+00AD на концах строк нет | resolveHyphenation→styledText→drawText(layout) без дорисовок: чернила дефиса в дефис-ячейке у края строки; ink правее width+3 == 0; bareShyEnds == 0; выполняется под START и JUSTIFY. | pending | reflow/impl/src/jvmTest/kotlin/ru/kyamshanov/notepen/reflow/ui/WordParityHyphenRenderTest.kt | PASS |
| RC-REFLOW-020 | 🟠 | 1 | unit | Обмер пагинации == рендер-путь: BlockHeightCalculator.measure и resolve+styledText (с живой фоновой подсветкой выделения) дают одинаковые height/lineBottoms | Высота и lineBottoms совпадают точно; фоновые anchor-спаны не меняют метрик (активное выделение не двигает пагинацию). | pending | reflow/impl/src/jvmTest/kotlin/ru/kyamshanov/notepen/reflow/ui/WordParityHyphenRenderTest.kt | PASS |
| RC-REFLOW-021 | 🟡 | 1 | unit | Пин поведения Skia при выключке: JUSTIFY рвёт строки по тем же точкам, что START (включая U+00AD); строки с пробелами растянуты до ширины колонки, строка без пробелов — нет | D1: совпадение концов строк START/JUSTIFY и ≥1 разрыв по шию под JUSTIFY; D2: getLineRight ≈ width для строк с пробелом. Падение после апгрейда Skiko = сигнал пересмотреть ReaderHyphenPromotion. | pending | reflow/impl/src/jvmTest/kotlin/ru/kyamshanov/notepen/reflow/ui/JustifyShyBreakDiagnosticTest.kt | PASS |
...
| RC-RENDER-013 | 🟠 | 1 | unit | bufferedRenderRange: в SPREAD буфер выровнен по разворотам и берёт соседние развороты целиком; в SINGLE — ±bufferPages | Spread window snaps to spread bounds and prefetches whole adjacent spreads (0..5 for visible 2..3); single extends by bufferPages, clamped to doc bounds. | WIP | rendering/impl/src/commonTest/kotlin/ru/kyamshanov/notepen/pdfviewer/PdfPagesLayoutTest.kt:509 | PEND |
| RC-RENDER-014 | 🟠 | 1 | unit | renderPriorityOrder ставит видимые страницы раньше буферных в окне растеризации | Visible indices are emitted before buffer indices (window 1..4 / visible 2..3 → [2,3,1,4]); empty window → empty list. | WIP | rendering/impl/src/commonTest/kotlin/ru/kyamshanov/notepen/pdfviewer/PdfPagesLayoutTest.kt:528 | PEND |
| RC-RENDER-015 | 🟠 | 2 | visual | Разворот: правая (соседняя) страница не остаётся пустой при навигации (видимое раньше буфера + предзагрузка разворотами + кэш 10) | Navigating spreads keeps BOTH pages rendered; the right page is never a blank slot waiting behind a buffer render. Verified on desktop (F4). | WIP | → AV-RENDER-07 | PEND |
...
| RC-SYNC-013 | 🟠 | 1 | unit | Тихий отказ LAN (VPN / ACCESS_LOCAL_NETWORK) классифицируется и даёт actionable-сообщение, а не «хост офлайн» | classifyConnectFailure maps EPERM/permission-denied (anywhere in the cause chain, beating timeout) to LOCAL_NETWORK_BLOCKED, refused/timeout to their kinds; ConnectFailureMessages appends VPN/permission hints from LocalNetworkHealth, except for PAIRING_REJECTED. | uncommitted 2026-06-11 | sync/src/commonTest/.../ConnectFailureClassifierTest.kt; qr-connect/src/commonTest/.../ConnectFailureMessagesTest.kt | PASS |
...
- 2026-06-12 — REFLOW: hyphen promotion (Word-parity выключка по ширине): добавлены RC-REFLOW-018..021; RC-REFLOW-016 RETIRED (дорисовка дефиса поверх раскладки удалена, преемник RC-REFLOW-019).
