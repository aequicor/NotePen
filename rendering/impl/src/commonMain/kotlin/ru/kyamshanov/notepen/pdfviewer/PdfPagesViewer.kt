...
expect class PdfViewerState {
    ...
    /**
     * Запрашивает перецентровку на странице [pageIndex], применяемую после
     * ближайшего пере-layout'а (смена режима разворота [spreadMode] / разделения
     * страниц). Ряд раскладки центрируется по горизонтали на свежем layout'е (в
     * развороте — пара страниц целиком), а верх [pageIndex] прижимается к верху
     * вьюпорта. Содержимое НЕ «масштабируется на весь экран».
     *
     * [zoomFactor] домножается на текущий [zoom] (с клампом в допустимый диапазон)
     * перед центрированием; `1f` (по умолчанию) — масштаб сохраняется как есть
     * (книжный разворот). Для разделения страниц передаётся `0.5f` при ВКЛючении
     * (половина страницы занимает целую колонку — без компенсации она выглядела бы
     * вдвое крупнее своей части в целой странице) и `2f` при ВЫКЛючении (обратное
     * действие); так визуальный размер контента и штрихов при переключении не
     * скачет. Граница раздела ровно `0.5` (см. `SpreadSplit.GUTTER_X`), поэтому
     * множитель — точная компенсация.
     *
     * Нужно потому, что смена режима пере-строит layout асинхронно (через
     * [pages] / [spreadMode]); вызвать прокрутку прямо в обработчике переключения
     * нельзя — layout ещё старый, и страница «уезжает» вбок (не центрируется) и
     * прыгает по вертикали. Запрос откладывается до готового layout'а.
     */
    fun requestRecenter(
        pageIndex: Int,
        zoomFactor: Float = 1f,
