...
sealed class QrConnectError(open val message: String) {
    ...
    data class ServerStartFailed(override val message: String) : QrConnectError(message)

    /** The [ru.kyamshanov.notepen.sync.domain.port.SyncClient] could not establish a session. */
    data class ConnectFailed(override val message: String) : QrConnectError(message)
}
