...
class PdfPortsContractTest {
    ...
    @Test
    fun loaderReturnsDocumentWithCorrectInfo() =
        runBlocking {
    ...
    @Test
    fun loaderRecordsLastLoadedPath() =
        runBlocking {
            val loader = FakePdfDocumentLoader(singlePageInfo)
    ...
    @Test
    fun rendererReturnsDimensionsAsRequested() =
        runBlocking {
            val doc = FakePdfDocument(singlePageInfo)
    ...
    @Test
    fun rendererThrowsForNegativePageIndex() =
        runBlocking<Unit> {
            val doc = FakePdfDocument(singlePageInfo)
            val renderer = FakePdfPageRenderer()
    ...
    @Test
    fun rendererThrowsForPageIndexEqualToPageCount() =
        runBlocking<Unit> {
            val doc = FakePdfDocument(singlePageInfo)
            val renderer = FakePdfPageRenderer()
            assertFailsWith<IndexOutOfBoundsException> {
    ...
    @Test
    fun rendererWorksForLastValidPageIndex() =
        runBlocking {
            val info =
                PdfDocumentInfo(
                    pageCount = 3,
