...
class CheckAvailabilityUseCaseTest {
    ...
    private fun makeFile(id: String, uri: String) = RecentFile(
        id = id,
        uri = uri,
     ...
     */
    @Test
    fun emitsUpdateForEachFile() = runTestBlocking {
        val files = listOf(
            makeFile("id1", "uri1"),
            makeFile("id2", "uri2"),
            makeFile("id3", "uri3"),
     ...
     */
    @Test
    fun updatesRepositoryStatusForEachFile() = runTestBlocking {
     ...
     */
    @Test
    fun emptyList_emitsNothing() = runTestBlocking {
     ...
     */
    @Test
    fun check_timeoutExceeded_emitsFILE_ERROR() = runTest {
        val hangingChecker = object : FileAvailabilityChecker {
     ...
     */
    @Test
    fun check_maxConcurrency_limitedTo5() = runTest {
        var maxConcurrent = 0
        var currentConcurrent = 0
        val mutex = Mutex()

        val countingChecker = object : FileAvailabilityChecker {
            override suspend fun check(uri: String): AvailabilityStatus {
                mutex.withLock {
                    currentConcurrent++
