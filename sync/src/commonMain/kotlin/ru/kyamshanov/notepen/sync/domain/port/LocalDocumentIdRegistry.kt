 ...
 * вычислять documentId заново из локального пути. Хранимое значение — это
 * **content-addressed wire id** (`<basename>#<sha256-prefix>`), полученный с
 * хоста через каталог: tablet берёт готовый id хоста, а не пересчитывает по
 * локальному пути (после загрузки байты совпадают, поэтому sha256 совпал бы,
 * но lookup всё равно дешевле и не требует чтения файла на UI-потоке).
 * Регистрация выполняется после успешной загрузки/cache-hit в
 * `RemoteDocumentOpener`; lookup — при открытии файла в редакторе.
 *
