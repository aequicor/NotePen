...
/**
 * Пол радиуса завитка (px) — лишь чтобы не делить на ноль на полном перевороте. Держим МАЛЫМ: при
 * progress=1 остаточный завиток даёт подъём z≈2·EPS и горизонтальный сдвиг ≈π·EPS от идеального
 * зеркального «плашмя». При EPS=1.5 это ≈4.7px текстуры (≈2px экрана при ×2 супер-сэмпле) — лист
 * «не докручивался» до конца и текст прыгал на этот остаток при передаче живой странице. 0.1 делает
 * остаток суб-пиксельным (арка тоньше шага меша, ни одна вершина в неё не попадает ⇒ резкий сгиб,
 * лист ложится ровно плашмя). Меньше делать незачем — деление и так безопасно.
 */
private const val EPS_RADIUS = 0.1f
