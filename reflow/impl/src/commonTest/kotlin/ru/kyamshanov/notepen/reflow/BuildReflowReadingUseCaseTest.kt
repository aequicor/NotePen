...
class BuildReflowReadingUseCaseTest {
    ...
    private val extractor = object : PdfReflowExtractor {
    ...
    @Test
    fun `maps page strokes to text highlights`() = runTest {
        val strokeOverHello = DrawingPath(
            points = listOf(
                DrawingPoint(52f / PAGE_WIDTH_PT, 103f / PAGE_HEIGHT_PT),
                DrawingPoint(78f / PAGE_WIDTH_PT, 107f / PAGE_HEIGHT_PT),
            ),
        )
        val reading = BuildReflowReadingUseCase(extractor)(
            path = "any.pdf",
    ...
    @Test
    fun `no strokes yields no highlights`() = runTest {
    ...
    @Test
    fun `strokes missing text yield no highlights`() = runTest {
        val strokeFarAway = DrawingPath(
