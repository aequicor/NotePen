...
class FileHistoryRepositoryDesktopTest {
    ...
    @Test
    fun `getAll excludes annotation sidecar entries`() =
        runTest {
    ...
    @Test
    fun `upsert then getAll returns entry sorted by openedAt desc`() =
        runTest {
    ...
    @Test
    fun `upsert with same uri does not create duplicate`() =
        runTest {
    ...
    @Test
    fun `upsert 21 entries causes eviction to max 20`() =
        runTest {
            for (i in 1..21) {
    ...
    @Test
    fun `rollbackUpsert removes entry by uri`() =
        runTest {
    ...
    @Test
    fun `upsert persistsAcrossReinstantiation`() =
        runTest {
    ...
    @Test
    fun `upsert_concurrent_noDuplicates`() =
        runTest {
            val jobs =
                (1..5).map { i ->
                    launch {
