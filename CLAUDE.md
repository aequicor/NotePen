...
- Screenshot tests use **Roborazzi + Compose Desktop UI test** in `:reflow:impl` (`jvmTest`). Record/verify with the dedicated tasks: `./gradlew :reflow:impl:recordRoborazziJvm` / `:verifyRoborazziJvm`. **Plain `jvmTest` (and therefore `./gradlew check`) does NOT compare goldens** — without a `roborazzi.test.*` mode `captureRoboImage` is a no-op, so the pixel check is *not* part of the merge gate today. The goldens are also **not yet deterministic across machines**: the reader renders with system font families (skiko resolves them from OS fonts, so macOS ≠ Linux CI), and the Baranovskaya fixtures additionally need a local, uncommitted PDF (they skip in CI). Treat the snapshots as a **local/advisory** check, recorded and verified by hand on one machine, until determinism (e.g. a bundled font via an isolated `FontFamily.Resolver` to avoid the skiko `registerTypeface` crash) lands.
